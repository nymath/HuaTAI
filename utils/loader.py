...
class TsDataSet(Dataset):
    ...
    def __init__(
        ...
        self.X_3D = np.concatenate(
            [
                self.X_2D_padded[index: index + self.offset][np.newaxis, ...]
                for index in range(len(self.X_2D))
            ],
            axis=0)
    ...
    def __getitem__(self, index) -> typing.Tuple[
        typing.Union[np.ndarray, torch.Tensor], typing.Union[np.ndarray, torch.Tensor]]:
